...
class InAppEventsTest {
    ...
    @Test
    fun `InAppMessageReceived publishes via SharedFlow`() {
        runBlocking {
    ...
    @Test
    fun `InAppMessageDismissed publishes with messageId and reason`() {
        runBlocking {
            Pyrx.publishEvent(
                PyrxEvent.InAppMessageDismissed(messageId = "x", reason = "user_dismissed"),
            )
    ...
    @Test
    fun `InAppMessageDismissed reason can be null`() {
        runBlocking {
    ...
    @Test
    fun `7-event taxonomy — every subtype reachable on the stream`() {
        runTest {
            Pyrx.resetForTesting()
            val all =
                listOf<PyrxEvent>(
    ...
    @Test
    fun `exhaustive when on 7-event sum compiles without else`() {
        runTest {
