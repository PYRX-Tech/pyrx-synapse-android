...
### 1. Add placeholder credentials → real credentials via Gradle properties

The sample reads `workspaceId`, `apiKey`, and `baseUrl` from `BuildConfig`
fields generated by `sample-app/build.gradle.kts` at compile time. Defaults
point at production with placeholder values that are guaranteed-rejected
by the backend (the app boots — config validation passes — but every event
POST fails until you swap them).

The build script reads via `providers.gradleProperty(...)`, which resolves
from `gradle.properties` (project-local or user-global), `-P` CLI overrides,
and `ORG_GRADLE_PROJECT_*` env vars. It does **NOT** read `local.properties`
— that file is reserved for `sdk.dir` and a small set of Android Gradle
Plugin keys.

**Recommended: override via `~/.gradle/gradle.properties`** (user-global,
never committed, survives clean checkouts):

```properties
# ~/.gradle/gradle.properties
pyrx.workspaceId = 11111111-2222-3333-4444-555555555555
pyrx.apiKey      = psk_test_<32-hex from Synapse dashboard → /settings/api-keys>
pyrx.baseUrl     = https://synapse-events.pyrx.tech
```

Then `chmod 600 ~/.gradle/gradle.properties` so the key isn't readable
by other users on the machine.

For local push testing with a Cloudflare Tunnel pointing at your dev box,
use a **first-level** `*.pyrx.tech` subdomain like
`pyrx.baseUrl = https://synapse-dev.pyrx.tech`. Cloudflare Universal SSL
does NOT cover second-level wildcards (e.g.
`dev.synapse-events.pyrx.tech` will fail the TLS handshake).

After editing `gradle.properties`, rebuild:

```bash
JAVA_HOME=$(/usr/libexec/java_home -v 17) ./gradlew :sample-app:assembleDebug
```

> **Never commit real credentials.** The user-global
> `~/.gradle/gradle.properties` lives outside any git repo. The
> BuildConfig pattern keeps real keys out of the binary's source tree
> even when bundled into the APK.
