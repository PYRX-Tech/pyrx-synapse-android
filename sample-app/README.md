...
### 1. Add placeholder credentials → real credentials via `local.properties`
...
**Override via `local.properties`** (in the repo root, gitignored by default):
...
use `pyrx.baseUrl = https://dev.synapse-events.pyrx.tech` (see "Local
push notification testing" below).

After editing `local.properties`, rebuild:

```bash
JAVA_HOME=$(/usr/libexec/java_home -v 17) ./gradlew :sample-app:assembleDebug
```

> **Never commit real credentials.** `local.properties` is in the standard
> Android `.gitignore`. The BuildConfig pattern keeps real keys out of
> the binary's source tree even when bundled into the APK.
